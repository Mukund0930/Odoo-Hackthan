...
class EventRsvp(Resource):
    @jwt_required(optional=True) # Allow non-logged-in users if guest details provided
    @ns.doc(security='jwt')
    ...
    def post(self, event_id):
        ...
        event = Event.query.filter_by(id=event_id, status='APPROVED').first_or_404(description='Event not found or not approved for RSVPs.')
        data = ns.payload
        current_user_id = get_jwt_identity() # Will be None if no token
        ...
        if current_user_id:
            ...
            # If user is logged in, check if they already RSVPd
            existing_rsvp = Rsvp.query.filter_by(event_id=event.id, user_id=user.id).first()
            if existing_rsvp:
                return {'message': 'You have already RSVPd to this event.'}, HTTPStatus.BAD_REQUEST
        else: # Guest RSVP
            if not guest_name or not guest_email: # Phone is optional
                return {'message': 'Name and Email are required for guest RSVPs.'}, HTTPStatus.UNAUTHORIZED
            # Optionally, check if a guest with this email already RSVPd
            existing_guest_rsvp = Rsvp.query.filter_by(event_id=event.id, guest_email=guest_email, user_id=None).first()
            if existing_guest_rsvp:
                 return {'message': f'Guest with email {guest_email} has already RSVPd to this event.'}, HTTPStatus.BAD_REQUEST


        rsvp = Rsvp(
            event_id=event.id,
            user_id=user_id_for_rsvp,
            guest_name=guest_name if not user_id_for_rsvp else None,
            guest_email=guest_email if not user_id_for_rsvp else None,
            guest_phone=guest_phone if not user_id_for_rsvp else None,
